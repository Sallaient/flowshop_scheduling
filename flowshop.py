    ...
    for machine in range(nb_mach):
        ...
        # Dès le 2ème job, on est dans ce cas
            ...
            if machine == 0:
                ...
                job['début'][machine] = max(
                    ordo['disponibilité'][machine], ordo['disponibilité'][machine-1])
            ordo['disponibilité'][machine] = job['début'][machine] + \
                job['durée'][machine]
...
def creer_ordo_liste_jobs(nb_machines, liste_jobs):
    ''' Renvoie un ordonnancement avec les jobs de la liste 'liste_jobs' 
        ordonnancés à la suite.
    '''
...
def eval(ordo, liste_jobs):
    ...
    LB2 = []
    for machine in range(nb_machines):
        dates_au_plus_tot_jobs = sorted(
            [(date_dispo(machine, job), idx, job) for idx, job in enumerate(liste_jobs)])
        ...
        makespan = makespan_probleme_1_rj_cmax(
            dates_au_plus_tot, [job['durée'][machine] for job in jobs])

        dates_de_latence = [duree_latence(
            machine, job, nb_machines) for job in liste_jobs]
        ...
        for i in range(len(places)+1):  # on crée un sommet pour chaque nouvel ordonnancement
            ...
            if new_non_places.__len__() == 0:  # évalutation
                ...
                if new_eval <= val_solution:
                    numero += 1
                    s = creer_sommet(new_eval, new_places,
                                     new_non_places, numero)
                    heapq.heappush(arbre, s)
